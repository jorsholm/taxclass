...
ranks <- c("Class", "Order", "Family", "Subfamily", "Tribe", "Genus", "Species")
...
result_BayesANT <-
  read.table(paste0("models/results_bayesant_finbol_gbol/bayesant_test",
                    shorttxt, "_finbol-gbol_", "species", ".txt"),
             header = T) |>
  dplyr::arrange(ID)
...
thinning <- tidyr::expand_grid(
    thinrank = c("family", "subfamily", "tribe", "genus", "species"),
    thinunit = c("num", "taxa")
) |>
    dplyr::filter(!(thinrank == "species" & thinunit == "num")) |>
    glue::glue_data("thin_{thinrank}_by_{thinunit}") |>
    c("all")

for (thin in thinning) {
    result_epatax[[thin]] <-
        read.table(
            glue::glue(
                "models/results_epa_finbol_gbol/epa_test{shorttxt}_finbol-gbol_{level}_{thin}.txt"
            ),
            header = T) |>
        dplyr::arrange(ID)
    result_epatax[[thin]] <- rename_PROTAX_output(result_epatax[[thin]])

        # all(stringr::str_to_lower(colnames(result_epatax)) ==
        #       stringr::str_to_lower(colnames(result_BayesANT)))
    colnames(result_epatax[[thin]])[-1] <-
        colnames(result_epatax[[thin]])[-1] |>
        chartr(old = "_", new = " ") |>
        stringr::str_to_title() |>
        chartr(old = " ", new = "_") |>
        sub(paste0("^(", paste(ranks, collapse = "|"), ")$"), "Taxon_\\1", x = _)
    result_epatax[[thin]] <- tidyr::pivot_longer(
        result_epatax[[thin]],
        -1,
        names_sep = "_",
...
# Some quick checks
if(!length(unique(lapply(result_epatax, function(x) colnames(x)))) == 1) print("One of the results data frames have wrong column names.")
...
data_true_long <- tidyr::pivot_longer(data_true, -1, names_to = "Rank", values_to = "True_Taxon")

library(ggplot2)

dplyr::bind_rows(result_epatax, .id = "thinning") |>
    dplyr::left_join(data_true_long, by = c("ID", "Rank")) |>
    dplyr::arrange(Prob) |>
    dplyr::mutate(
        Rank = factor(Rank, levels = ranks),
        `Cumulative Probability` = cumsum(Prob)/dplyr::n(),
        `Cumulative Correct` = cumsum(Taxon == True_Taxon)/dplyr::n(),
        .by = c(thinning, Rank)
    ) |>
    ggplot(aes(x = `Cumulative Probability`, y = `Cumulative Correct`, group = Rank, color = Rank)) +
    geom_line() +
    facet_wrap(~thinning)

ggsave("../plots/epa_thinning_calibration.pdf", width = 8, height = 6)
...
dplyr::mutate(
    data_true_long,
    novel = endsWith(True_Taxon, "new"),
    novel_rank = ifelse(novel, sub(".+_([^_]+)_new", "\\1", True_Taxon), NA)
) |>
    dplyr::left_join(
        dplyr::bind_rows(result_epatax, .id = "thinning"),
        y = _,
...
dplyr::bind_rows(
    all = pre_accuracy |>
        dplyr::summarize(Accuracy = sum(Taxon == True_Taxon)/dplyr::n(), .by = c(Rank, thinning)),
    novel = pre_accuracy |>
        dplyr::filter(Rank == novel_rank) |>
        dplyr::summarize(Accuracy = sum(Taxon == True_Taxon)/dplyr::n(), .by = c(Rank, thinning)),
    known = pre_accuracy |>
        dplyr::filter(novel == FALSE) |>
        dplyr::summarize(Accuracy = sum(Taxon == True_Taxon)/dplyr::n(), .by = c(Rank, thinning)),
    .id = "subset"
) |>
    dplyr::mutate(Rank = factor(Rank, levels = ranks)) |>
    ggplot(aes(x = Rank, y = Accuracy, group = thinning, color = thinning)) +
    geom_line() +
    facet_wrap(~subset)

ggsave("../plots/epa_thinning_accuracy.pdf", width = 8, height = 6)
