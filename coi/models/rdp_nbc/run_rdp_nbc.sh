...
gawk -F'[|]' '
  BEGIN {
    OFS = "*"
    split("class,order,family,subfamily,tribe,genus,species", ranknames, ",")
  ...
  END {
    ...
    n = 1
    for (r = 1; r <= nranks; r++) {
      #initialize empty subarray by assigning an element and deleting
      id[r][0] = 0
      delete id[r][0]

      for (i = 1; i <= ntaxa; i++) {
        if (! (label[i][r] in id[r])) {
          id[r][label[i][r]] = n
          print n, label[i][r], id[r-1][parent[i][r]], r, ranknames[r]
          n++
        }
      }
    }
