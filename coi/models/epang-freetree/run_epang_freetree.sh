#!/usr/bin/env bash
#SBATCH --job-name=epang-freetree
...
#SBATCH --array=4
#SBATCH --output=epang-freetree_%a.out
#SBATCH --error=epang-freetree_%a.out
...
# define file names
MODEL=epang-freetree
DATA=../../data
RESULTS=../../results/$MODEL
mkdir -p $RESULTS
TRAIN_TAXONOMY_FILE=$DATA/train_gappa.tax

cp $TRAIN_TAXONOMY_FILE taxonomy.txt
echo "outgroup	Chromadorea;Rhabditida;Rhabditidae;Rhabditidae_subfamily_Incertae_sedis;Rhabditidae_tribe_Incertae_sedis;Caenorhabditis;Caenorhabditis_elegans" >>taxonomy.txt
echo outgroup >outgroup.txt
# loop over nt and aa
...
do
  ...
  OUTGROUP_FILE=$DATA/outgroup_${ALPHABET}_aln.fasta
  ALIGN_FILE=$MODEL_DIR/train_outgroup_${ALPHABET}_aln.fasta

  rm -fr $MODEL_DIR
  mkdir -p $MODEL_DIR

  cat $TRAIN_FILE $OUTGROUP_FILE >$ALIGN_FILE
