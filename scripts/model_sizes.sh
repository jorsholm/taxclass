...
ls -s1 */models/blast/*.n?? | sed 's/^ *//' >>$OUTFILE
...
ls -s1 */models/idtaxa/*.rds | sed 's/^ *//' >>$OUTFILE
...
du -s */models/protax-a/protax-a_*_tax_1 | tr "\t" " " >>$OUTFILE

# RDP-NBC: model is multiple files in a directory
du -s */models/rdp_nbc/rdp_nbc_1 | tr "\t" " " >>$OUTFILE
