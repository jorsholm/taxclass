...
do
  ...
  echo "ID	kingdom	Prob_kingdom	phylum	Prob_phylum	"\
       "class	Prob_class	order	Prob_order	"\
       "family	Prob_family	genus	Prob_genus	species	Prob_species"\
       >$RESULT_FILE
...
do
  ...
  echo "ID	kingdom	Prob_kingdom	phylum	Prob_phylum	"\
       ...
       >$RESULT_FILE
  awk -F"[,;]"    'BEGIN {
       n=split("65,70,75,80,85,90,98", thresh)
     }
     NR>1 {printf "%s", $1
       sub(/tax=/, "")
       for (i = 1; i <= n; i++) {
